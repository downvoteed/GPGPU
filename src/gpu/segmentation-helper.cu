...
// TODO : actually use it and remove shared memory
/**
 * Compute LBP kernel side for the first frame (backgroud image).
 */
__global__ void calculate_lbp_kernel(uchar3* image, float* result, int width,
                                     int height)
{
    // declare shared memory buffer
    __shared__ int directions[8][2];
    // let the first thread initialize the shared data
    ...
    // make sure all threads in a block have the shared data before computation
    __syncthreads();
    ...
    {
        float center = 0.2989f * image[idy * width + idx].x
            + 0.5870f * image[idy * width + idx].y
            + 0.1140f * image[idy * width + idx].z;
        ...
        for (int i = 0; i < 8; i++)
        {
            ...
            float value = 0.2989f * image[y * width + x].x
                + 0.5870f * image[y * width + x].y
                + 0.1140f * image[y * width + x].z;
                                         ...
                                         int width, int height)
{
    ...
    value = 0.2989f * image[y * width + x].x + 0.5870f * image[y * width + x].y
        + 0.1140f * image[y * width + x].z;
...
/**
 * Compare two LBP values
 * @param lbp1 first LBP value
 * @param lbp2 second LBP value
 * @return the similarity between the two LBP values
 */
...
/**
 * Compute the fuzzy integral of two images
 * @param image1 current image processed
 * @param image2 background image
 * @param lbpBackground LBP of the background image
 */
                               ...
                               int height)
{
    ...
    // Compute the ratio of the two pixels
    // TODO: replace min and max with lib functions
    ...
    float final_result = coefficients[0] * 0.1f + coefficients[1] * 0.3f
        + coefficients[2] * 0.6f;
