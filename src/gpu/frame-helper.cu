                    ...
                    const std::string& output_path)
{
    ...
    // resolution
    std::cout << "Resolution: " << frameSize.width << "x" << frameSize.height
              << std::endl;
    // fps
    ...
    cudaMemcpy(d_image1, frame.ptr<uchar3>(), width * height * sizeof(uchar3),
    ...
    for (int y = 0; y < height; ++y)
    {
        ...
        {
            h_lbpBackground[y * width + x] =
                calculateLBP(frame.ptr<uchar3>(), x, y, width, height);
        }
    }

    cudaMemcpyAsync(d_lbpBackground, h_lbpBackground,
    ...
    do
    {
        ...
        cudaMemcpyAsync(d_image2, frame.ptr<uchar3>(),
                        width * height * sizeof(uchar3), cudaMemcpyHostToDevice,
                        stream1);
        fuzzy_integral<<<gridSize, blockSize, 0, stream1>>>(
            d_image1, d_image2, d_lbpBackground, d_result, width, height);

        if (isFrameRead)
        {
            isFrameRead = cap.read(frame);
            cudaMemcpyAsync(d_image1, d_image2, width * height * sizeof(uchar3),
                            cudaMemcpyDeviceToDevice, stream2);
        }

        cv::Mat processed_frame(height, width, CV_32F);
        cudaMemcpy2DAsync(processed_frame.ptr<float>(), width * sizeof(float),
                          d_result, width * sizeof(float),
