...
namespace frame_helper
{
     ...
     */
    const cv::Mat& readFrame(const std::string& path, const bool gray)
    {
     ...
     * @return The frames
     */
    const frames_vector* readFrames(const std::string& path,
     ...
     */
    void saveFrames(const std::string& path, frames_ref& frames, const int fps)
    {
        cv::Size frameSize(frames[0]->cols, frames[0]->rows);
        cv::VideoWriter video(path, cv::VideoWriter::fourcc('m', 'p', '4', 'v'),
                              fps, frameSize);
        if (!video.isOpened())
        {
            BOOST_LOG_TRIVIAL(error) << "Could not open the video: " << path;
